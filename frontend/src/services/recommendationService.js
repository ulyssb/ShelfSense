 ...
 */
export async function generateRecommendations(currentBooks, preferredGenres) {
  try {
    console.log("Generating recommendations with:", { currentBooks, preferredGenres })
    const aiRecommendations = await recommendBooks(currentBooks, preferredGenres)
    ...
    logError(error, 'RecommendationService', { currentBooks, preferredGenres })
